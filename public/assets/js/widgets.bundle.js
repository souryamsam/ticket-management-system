...
var KTChartsWidget22 = function () {
    // Private methods
    var initChart = function(chartSelector, data, labels, initByDefault) {
        var element = document.querySelector(chartSelector);
        ...
        var options = {
            ...
            labels: labels, // Custom labels for the two values
            legend: {
                show: false,
            },
            fill: {
                type: 'solid', // Ensure solid fill
            }     
        };
    ...
    return {
        init: function () {           
            initChart(
                ...
                [207, 133], // Two data points: Open Dockets and Closed Dockets
                ['Open Dockets', 'Closed Dockets'], // Labels
                true
            );
        }
    };
...
var KTChartsWidget36 = function () {
	...
	var initChart = function(chart) {
		...
		var inboundData = [65, 80, 70, 60, 50, 85, 90];
		var outboundData = [90, 110, 100, 95, 85, 115, 120];
		...
		var options = {
			...
			xaxis: {
				categories: last7Days, // Use the generated last 7 days
				axisBorder: {
					show: false,
				},
				axisTicks: {
...
var KTTablesWidget30 = function () {
    ...
    const initDatatable = () => {
        // Get subtable template
        const subtable = document.querySelector('[data-kt-table-widget-30="subtable_template"]');
        template = subtable.cloneNode(true);
        template.classList.remove('d-none');
    ...
    // Search Datatable --- official docs reference: https://datatables.net/reference/api/search()
    var handleSearchDatatable = () => {
        const filterSearch = document.querySelector('[data-kt-table-widget-30="search"]');
        filterSearch.addEventListener('keyup', function (e) {
            datatable.search(e.target.value).draw();
        });
    }
    ...
    // Handle status filter
    const handleStatusFilter = () => {
        const select = document.querySelector('[data-kt-table-widget-30="filter_status"]');
    ...
    // Handle action button
    const handleActionButton = () => {
        const buttons = document.querySelectorAll('[data-kt-table-widget-30="expand_row"]');
        ...
        buttons.forEach((button, index) => {
            button.addEventListener('click', e => {
                ...
                // Handle subtable expanded state
                if (row.classList.contains('isOpen')) {
                    while (row.nextSibling && row.nextSibling.getAttribute('data-kt-table-widget-30') === 'subtable_template') {
                        row.nextSibling.parentNode.removeChild(row.nextSibling);
                    }
                    row.classList.remove(...rowClasses);
                    button.classList.remove('active');
                } else {
                    populateTemplate([], row); // Replace with appropriate data
                    row.classList.add(...rowClasses);
                    button.classList.add('active');
                }
            });
    ...
    const populateTemplate = (data, target) => {
        data.forEach((d, index) => {
            ...
            // Populate elements with data (replace with actual data logic)
    ...
    // Reset subtable
    const resetSubtable = () => {
        const subtables = document.querySelectorAll('[data-kt-table-widget-30="subtable_template"]');
        subtables.forEach(st => {
            st.parentNode.removeChild(st);
        });
        ...
        rows.forEach(r => {
            r.classList.remove('isOpen');
            if (r.querySelector('[data-kt-table-widget-30="expand_row"]')) {
                r.querySelector('[data-kt-table-widget-30="expand_row"]').classList.remove('active');
            }
        });
    }
    ...
    return {
        init: function () {
            table = document.querySelector('#kt_table_widget_30_table');
